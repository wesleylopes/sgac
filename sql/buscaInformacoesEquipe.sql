DROP PROCEDURE IF EXISTS  buscaInformacoesEquipe;
DELIMITER $$
CREATE PROCEDURE buscaInformacoesEquipe(
...
SET buscaInformacoesEquipeConsolidadoveiculosMovimento = (SELECT count(*) 
					    FROM movimento_veiculos b 
	                    WHERE b.PRODUTO like CONCAT('%', p_tipoCombustivel, '%')
                       AND CENTRO_CUSTO like CONCAT('%', p_equipe,'%')
						AND b.CENTRO_RESULTADO in(SELECT DISTINCT(CENTRO_RESULTADO ) FROM movimento_veiculos a 
...
-- Abertura do cursor
  ...
  SELECT 
    ...
    FORMAT(v_custo_combustivel_km,0) as CUSTO_COMBUSTIVEL_KM;
    END IF;
END $$
